...
const GET_InviteCodes = async (_: Request, res: Response): Promise<void> => {
  try {
    ...
    if (difference <= window) {
      ...
      log.info("Hidden invite codes for account %s. %s seconds left for releasing invite codes.", user._id, waitTime);
      res
        .status(statusCodes.clientInputError)
        .json(new ErrorResponse(errorMessages.inviteCodesHidden, { waitTime: Math.ceil(waitTime) }));
      return;
    }
