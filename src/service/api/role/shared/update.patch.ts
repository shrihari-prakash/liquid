...
const POST_Update = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isSystemRole) {
      return res.status(statusCodes.clientInputError).json(new ErrorResponse(errorMessages.systemRoleUpdate));
    }
    ...
    if (!existingRole) {
      return res.status(statusCodes.clientInputError).json(new ErrorResponse(errorMessages.notFound));
    }
    req.body.id = existingRole.id;
    delete req.body.target;
    const role = await Role.updateRole(req.body);
    return res.status(statusCodes.success).json(new SuccessResponse({ role }));
  } catch (err) {
    log.error(err);
