...
> **_NOTE:_** If you are using nginx as reverse proxy and you do not see cookies working or if you get error `Server error: handle() did not return a user object` while logging in, add `proxy_set_header X-Forwarded-Proto $scheme;` to your server -> location settings of nginx.
### Development
1. Run `npm i`.
